...
{
  private $menuActive = "petugas-lapang";
  private $submnActive = "";
  public function index(Request $request) {
    $this->data['menuActive'] = $this->menuActive;
    $this->data['submnActive'] = $this->submnActive;
    if ($request->ajax()) {
        $data = User::where('level_user', '2')->get();
    ...
    return view('petugas-lapangan.main')->with('data', $this->data);
  }

  public function form(Request $request) {
    try {
      $data['data'] = (!empty($request->id)) ? User::find($request->id) : "";
      $content = view('petugas-lapangan.form', $data)->render();
  ...
  public function store(Request $request) {
    ...
    try {
        ...
        $newpetugas = new PetugasLapangan;
