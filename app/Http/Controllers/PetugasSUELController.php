...
class PetugasSUELController extends Controller
{
  private $menuActive = "petugas-su-el";
  private $submnActive = "";
  public function index(Request $request) {
    $this->data['menuActive'] = $this->menuActive;
    $this->data['submnActive'] = $this->submnActive;
    if ($request->ajax()) {
        $data = User::where('level_user', '4')->get();
    ...
    return view('petugas-su-el.main')->with('data', $this->data);
  }

  public function form(Request $request) {
    try {
      $data['data'] = (!empty($request->id)) ? User::find($request->id) : "";
      $content = view('petugas-su-el.form', $data)->render();
