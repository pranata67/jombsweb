...
class PetugasPemetaanController extends Controller
{
  private $menuActive = "pemetaan";
  private $submnActive = "";
  public function index(Request $request) {
    $this->data['menuActive'] = $this->menuActive;
    $this->data['submnActive'] = $this->submnActive;
    if ($request->ajax()) {
        $data = User::where('level_user', '3')->get();
    ...
    return view('petugas-pemetaan.main')->with('data', $this->data);
  }

  public function form(Request $request) {
    try {
      $data['data'] = (!empty($request->id)) ? User::find($request->id) : "";
      $content = view('petugas-pemetaan.form', $data)->render();
  ...
  public function store(Request $request) {
    ...
    try {
        ...
        $newpetugas = new PetugasPemetaan;
